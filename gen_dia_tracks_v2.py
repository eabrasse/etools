...
dia_keys = 'cor','hadv','vadv','prsgrd','accel','vvisc'
...
#read particles into a dict format
# particlelist = range(np.sum(tma))
particlelist = [30, 100, 150, 175]
#initialize pdict
pdict = dict()
...
for day in range(Ndays):
    ...
    for hr in range(hrs):
        ...
        for particle in particlelist: 
            ...
            u_cor = ds_dia['u_cor'][0,indz_u,indy_u,indx_u]
            ...
            v_cor = ds_dia['v_cor'][0,indz_v,indy_v,indx_v]
            
            # u_sign = int(np.sign(u_cor))
            # v_sign = int(np.sign(v_cor))
            # number = np.count_nonzero([u_sign,v_sign])
            # if number<2:
            #      quadrant = 0
            #      #funky formula, but it should identify position correctly
            #      theta = -0.5*(u_sign-1)*np.pi+0.5*v_sign*(v_sign-1)*np.pi
            # if number==2:
            #      theta = np.arctan(v_cor/u_cor)+0.5*(u_sign-1)*np.pi     
            
            theta = math.atan2(v_cor,u_cor)
            
            # p['Da']['cor'][tt] = u_cor*np.cos(theta) + v_cor*np.sin(theta)
            # p['Dc']['cor'][tt] = -u_cor*np.sin(theta) + v_cor*np.cos(theta)
