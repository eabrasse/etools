...
#testing
f_list = f_list[45:] # control number of days
...
#then consider the shelf box, mouth as single values by adding up all the transport
# per salinity bin from all the spatial cells
rq_mouth_sum = np.sum(rq_mouth,axis=1)
rq_shelfbox_sum = np.sum(rq_plume,axis=1)+np.sum(rq_shelf,axis=1)+np.sum(rq_south,axis=1)

# then form the cumulative sum (the functon Q(s))
# qcs_plume = np.cumsum(rq_plume, axis=0)
# qcs_shelf = np.cumsum(rq_shelf, axis=0)
qcs_mouth = np.cumsum(rq_mouth_sum, axis=0)
qcs_shelfbox = np.cumsum(rq_shelfbox_sum,axis=0)
...
imax_m = np.argmax(qcs_mouth)
imax_sb = np.argmax(qcs_shelfbox)
#Sm_cutoff = sedges[imax_m]
#Ssb_cutoff = sedges[imax_sb]
...
Qin_shelf = np.sum(rq_shelf[:imax_sb, :], axis=0)
Qout_shelf = np.sum(rq_shelf[imax_sb:, :], axis=0)
QSin_shelf = np.sum(rqs_shelf[:imax_sb, :], axis=0)
QSout_shelf = np.sum(rqs_shelf[imax_sb:, :], axis=0)

Qin_mouth = np.sum(rq_mouth[:imax_m, :], axis=0)
Qout_mouth = np.sum(rq_mouth[imax_m:, :], axis=0)
QSin_mouth = np.sum(rqs_mouth[:imax_m, :], axis=0)
QSout_mouth = np.sum(rqs_mouth[imax_m:, :], axis=0)
...
Qin_plume = np.sum(rq_plume[:imax_sb, :], axis=0)
Qout_plume = np.sum(rq_plume[imax_sb:, :], axis=0)
QSin_plume = np.sum(rqs_plume[:imax_sb, :], axis=0)
QSout_plume = np.sum(rqs_plume[imax_sb:, :], axis=0)

Qin_south = np.sum(rq_south[:imax_sb, :], axis=0)
Qout_south = np.sum(rq_south[imax_sb:, :], axis=0)
QSin_south = np.sum(rqs_south[:imax_sb, :], axis=0)
QSout_south = np.sum(rqs_south[imax_sb:, :], axis=0)
...
out_fn = out_dir + 'TEFbalancev2_aestus2_unstrat_south.p'
