...
for t in range(ndays):
    ...
    if t==0:
        ...
        G, S, T = zrfun.get_basic_info(oceanfn)
        lonr = G['lon_rho'][:]
        ...
        a_ll = [-1.0, 1.2, latr.min()+0.25, latr.max()-0.25]
    ...
    rl = np.ma.masked_where(maskr==0, rho[-1,:,:])
    ...
    for lat in range(len(l)):
        if l_list[lat]==45.0:
            bb = efun.box_inds(-0.65,1.5,44.0,46.0,oceanfn)
        else:
            bb = efun.box_inds(-0.65,0.0,44.0,46.0,oceanfn)
